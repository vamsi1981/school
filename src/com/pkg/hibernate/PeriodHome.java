...
public class PeriodHome {
	...
	public Period merge(Period detachedInstance) {
		...
		try {
			Period result = (Period) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Period findById(java.lang.String id) {
		log.debug("getting Period instance with id: " + id);
		try {
			Period instance = (Period) sessionFactory.getCurrentSession().get(
					"com.pkg.hibernate.Period", id);
			if (instance == null) {
	...
	public List findByExample(Period instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("com.pkg.hibernate.Period")
